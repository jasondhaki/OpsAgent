...
export default async function TicketPage({ params }: PageProps<'/tickets/[id]'>) {
  ...
  const { data: t } = await sb.from('tickets').select('*, customer:customers(email)').eq('id', id).eq('org_id', m.orgId).maybeSingle();
  if (!t) notFound(); // RLS: non-members get nothing
  const [{ data: messages }, { data: drafts }, { data: events }] = await Promise.all([
    sb.from('messages').select('id, direction, from_address, body_text, created_at').eq('ticket_id', id).order('created_at'),
    sb.from('drafts').select('id, version, body, author, citations, validator, groundedness, meta, created_at').eq('ticket_id', id).order('version', { ascending: false }),
    sb.from('ticket_events').select('id, actor, type, data, created_at').eq('ticket_id', id).order('created_at'),
  ]);
  ...
  const { data: chunks } = citations.length
    ? await sb.from('kb_chunks').select('id, content').in('id', citations.map((c) => c.chunkId))
    : { data: [] };

  return (
    <div className="space-y-4">
      <div>
        <Link href="/queue" className="text-sm text-muted-foreground hover:underline">← Queue</Link>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <ReviewPanel
                ...
                status={t.status}
                canReview={hasRole(m.role, 'reviewer')}
                isOwner={hasRole(m.role, 'owner')}
              />
            </CardContent>
          </Card>
